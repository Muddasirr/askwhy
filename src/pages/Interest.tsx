...
const Interest = () => {
  ...
  const handleVote = (id: number, vote: "interested" | "not-interested", title: string) => {
    setTopics(prev => {
      ...
      if (nextVote === "interested") dispatch(addTopic(id));
      else dispatch(removeTopic(id));

      const updated = prev.map(topic =>
        topic.id === id ? { ...topic, voted: nextVote } : topic
      );

      if (updated.filter(t => t.voted === "interested").length >= 5) {
  ...
  return isComplete ? (
    ...
    <div className="p-4 md:p-12">
      <div
        className="h-auto md:h-[90vh] px-4 md:px-24 rounded-xl md:rounded-[24px] shadow-[0px_0px_25px_0px_#0000001A_inset] overflow-auto py-4 md:py-2"
        style={{ backgroundColor: "#F8F1E7" }}
      >
        <OpeningModal
          phase="I"
          module="Module 1: Find your vibe"
          description="In this module, students will filter out content for themselves..."
          time="2:00"
          calculated="not"
          level="Beginner"
          src={"/opening11.svg"}
        ...
        <div className={`transition-all duration-300 ${showIntroModal ? "blur-sm pointer-events-none" : ""}`}>
          <ModuleHeader
            ...
            left={Math.max(5 - selectedCount, 0)}
          />

          <h1 className="text-black text-center text-[20px] md:text-[24px]">
            Pick 5 interests to continue
          </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-8 mt-4">
            {topics.map(topic => (
              <Card
                key={topic.id}
                className="p-4 bg-white border-gray-200 hover:shadow-md transition-shadow flex flex-col items-center justify-center text-center w-full"
              >
                <div>
                  <span
                ...
                <h3 className="font-normal text-[14px] md:text-[16px] text-[#130719] my-4 leading-tight">
                  {topic.title}
                </h3>
                ...
                <div className="flex w-full items-center justify-center">
                  <Button
                    ...
                    className="relative overflow-hidden font-normal text-[12px] bg-transparent px-0 py-0"
                    asChild
                  >
                    <motion.div
                      animate={{
                      ...
                      className="flex cursor-pointer items-center justify-center px-4 md:px-6 py-2 rounded-md"
                    >
                      <img
                        src={topic.voted !== "interested" ? "/love.svg" : "/whitelove.svg"}
